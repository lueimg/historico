...
class Persona {
    ...
    private function ConditionWhere()
    {
        ...
        if($filtros["filter"]== "ape"){
            ...
            $where  .=  "  and pg.id_grupo =".$filtros["value"]."  ";
    ...
    public function ListadoPersonas() {
        ...
        try{
            ...
            $cad = "select  pe.id,pe.nombre,pe.apellido_p,pe.apellido_m,pe.dni , pe.estado , ee.eecc
                ...
                left join webpsi.tb_persona_grupo pg on pg.id_persona = pe.id
                LEFT JOIN(
    ...
    public function getDataUsuarioAll($id)
    {
        ...
        try{
            $sql = "
            select   pe.id, pe.nombre ,pe.apellido_p, pe.apellido_m, pe.dni , pe.id_eecc, pe.estado,
            pg.id_grupo,
            GROUP_CONCAT(CONCAT_WS('-',pc.contacto,pc.defecto) SEPARATOR '|') numeros
            from webpsi.tb_persona pe
            inner join webpsi.tb_persona_contacto pc on pc.id_persona = pe.id
            inner join webpsi.tb_persona_grupo pg on pg.id_persona = pe.id
            inner join webpsi.tb_grupo g on g.id = pg.id_grupo
            where pe.id = $id;";
            $stmt = $cnx->query($sql);
